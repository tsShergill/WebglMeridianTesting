...
function BuildScene(rawRoomDataView) {
    ...
    while (count <= NumSectors) {
        ...
        if (CheckBit(Sectors[count].SectorFlags, 10) == 1) // Check if it's a sloped Floor
        {
            ...
            Sectors[count].FloorTextureOriginX = rawRoomDataView.getFloat32(offset + 16, true);
            Sectors[count].FloorTextureOriginY = rawRoomDataView.getFloat32(offset + 20, true);
            Sectors[count].FloorTectureAngle = rawRoomDataView.getFloat32(offset + 24, true);
        ...
        if (CheckBit(Sectors[count].SectorFlags, 11) == 1) // Check if it's a sloped Ceiling
        {
            ...
            Sectors[count].CeilingTextureOriginX = rawRoomDataView.getFloat32(offset + 16, true);
            Sectors[count].CeilingTextureOriginY = rawRoomDataView.getFloat32(offset + 20, true);
            Sectors[count].CeilingTectureAngle = rawRoomDataView.getFloat32(offset + 24, true);
    ...
    var left = 0.0;
    var top = 0.0;
    var oneOverCfloor = 0.0;
    var oneOverCceiling = 0.0;
    var LeafVertex = new Array();
    var leafcount = 1;


    while (count <= NumNodes) {
        NodeType = rawRoomDataView.getInt8(offset, true);

        if (NodeType == 1) { // Internal Node
            offset = offset + 35; // Skip this for now and move on
        }
        else if (NodeType == 2) { // Leaf Node
            LeafNodes[leafcount] = {
                boxNWx: rawRoomDataView.getInt32(offset + 1, true),
                boxNWy: rawRoomDataView.getInt32(offset + 5, true),
                boxSEx: rawRoomDataView.getInt32(offset + 9, true),
                boxSEy: rawRoomDataView.getInt32(offset + 13, true),
                SectoYrNum: rawRoomDataView.getInt16(offset + 17, true),
                NumOfPoints: rawRoomDataView.getInt16(offset + 19, true),
                LeafNodeOffsetStart: offset,
                FloorTexID: Sectors[rawRoomDataView.getInt16(offset + 17, true)].FloorBitmapNo,
                CeilingTexID: Sectors[rawRoomDataView.getInt16(offset + 17, true)].CeilingBitmapNo
            }

            offset = offset + 21; // Move the file offset on to the Points

            left = rawRoomDataView.getInt32(offset, true);
            top = rawRoomDataView.getInt32(offset + 4, true);

            oneOverCfloor = 1.0 / Sectors[LeafNodes[leafcount].SectoYrNum].FloorSlopecCoef;
            oneOverCceiling = 1.0 / Sectors[LeafNodes[leafcount].SectoYrNum].CeilingSlopecCoef;

            while (countb <= LeafNodes[leafcount].NumOfPoints) {
                ...
                if (LeafVertex[countb].x > left) {
                    left = LeafVertex[countb].x;
                }
                if (LeafVertex[countb].y > top) {
                    left = LeafVertex[countb].y;
                }

                if (CheckBit(Sectors[LeafNodes[leafcount].SectoYrNum].SectorFlags, 10) == 1) { // Get Floor height for this vertex
                    LeafVertex[countb].zf = (-Sectors[LeafNodes[leafcount].SectoYrNum].FloorSlopeaCoef * LeafVertex[countb].x - Sectors[LeafNodes[leafcount].SectoYrNum].FloorSlopebCoef * LeafVertex[countb].y - Sectors[LeafNodes[leafcount].SectoYrNum].FloorSlopedCoef) * oneOverCfloor;
                }
                else
                {
                    LeafVertex[countb].zf = Sectors[LeafNodes[leafcount].SectoYrNum].FloorHeight * 16.00;
                }

                if (CheckBit(Sectors[LeafNodes[leafcount].SectoYrNum].SectorFlags, 11) == 1) { // Get Ceiling Height for this vertex
                    LeafVertex[countb].zc = (-Sectors[LeafNodes[leafcount].SectoYrNum].CeilingSlopeaCoef * LeafVertex[countb].x - Sectors[LeafNodes[leafcount].SectoYrNum].CeilingSlopebCoef * LeafVertex[countb].y - Sectors[LeafNodes[leafcount].SectoYrNum].CeilingdCoef) * oneOverCceiling;
                }
                else {
                    LeafVertex[countb].zc = Sectors[LeafNodes[leafcount].SectoYrNum].CeilingHeight * 16.00;
                }
            ...
            if (LeafNodes[leafcount].FloorTexID > 0){
                MakeFloorCeiling(LeafVertex, 1, LeafNodes[leafcount].FloorTexID, Sectors[LeafNodes[leafcount].SectoYrNum], CheckBit(Sectors[LeafNodes[leafcount].SectoYrNum].SectorFlags, 10), top, left);
                }
            if (LeafNodes[leafcount].CeilingTexID > 0) {
                MakeFloorCeiling(LeafVertex, 2, LeafNodes[leafcount].FloorTexID, Sectors[LeafNodes[leafcount].SectoYrNum], CheckBit(Sectors[LeafNodes[leafcount].SectoYrNum].SectorFlags, 11), top, left);
            }
        ...
        LeafVertex = null;
        LeafVertex = new Array();
        count++;
        countb = 1;
    }

    // Now we have the BSP Tree, lets loop through again and build the room
...
function MakeFloorCeiling(LeafVertex, floorceiling, TexID, SectorData, sloped, top, left) {

    var a, b, c, d, texorigx, texorigy, texangle;
    var uv = new Array();

    if (floorceiling == 1) { // This is a floor so lets grab the floor vars
            a = SectorData.FloorSlopeaCoef;
            b = SectorData.FloorSlopebCoef;
            c = SectorData.FloorSlopecCoef;
            d = SectorData.FloorSlopedCoef;
            texorigx = SectorData.FloorTextureOriginX;
            texorigy = SectorData.FloorTextureOriginY;
            texangle = SectorData.FloorTectureAngle;
    }
    else if (floorceiling == 2) {
        a = SectorData.CeilingSlopeaCoef;
        b = SectorData.CeilingSlopebCoef;
        c = SectorData.CeilingSlopecCoef;
        d = SectorData.CeilingSlopedCoef;
        texorigx = SectorData.CeilingTextureOriginX;
        texorigy = SectorData.CeilingTextureOriginY;
        texangle = SectorData.CeilingTectureAngle;
    }

    var count = 1;
    geom = new THREE.Geometry();

    while (count < LeafVertex.length) {

        if (sloped == 1) { // Calculate UV with Slope
            uv[count] = {
                X: Math.abs(LeafVertex[count].y - top) - (texorigy << 4),
                Y: Math.abs(LeafVertex[count].x - left) - (texorigx << 4)
            };

        }
        else { // Calculate UV without Slope
            uv[count] = {
                X: Math.abs(LeafVertex[count].y - top) - (texorigy << 4),
                Y: Math.abs(LeafVertex[count].x - left) - (texorigx << 4)
            };
        ...
        if (floorceiling == 1) {
            geom.vertices.push(new THREE.Vector3(LeafVertex[count].y / 16, LeafVertex[count].zf / 16, LeafVertex[count].x / 16));
        }
        else {
            geom.vertices.push(new THREE.Vector3(LeafVertex[count].y / 16, LeafVertex[count].zc / 16, LeafVertex[count].x / 16));
        }

        if (count > 2) {

            geom.faces.push(new THREE.Face3(count, count - 1, 1));
    ...
    geomObj = new THREE.Mesh(geom, new THREE.MeshBasicMaterial({ color: 0x404040 }));
